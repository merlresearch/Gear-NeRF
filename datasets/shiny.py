...
from utils.ray_utils import get_ndc_rays_fx_fy, get_ray_directions_K
...
class ShinyDataset(LLFFDataset):
    ...
    def read_meta(self):
        ...
        with self.pmgr.open(os.path.join(self.root_dir, "hwf_cxcy.npy"), "rb") as f:
            hwfc = np.load(f)
        ...
        self.poses, self.poses_avg, self.bounds = correct_poses_bounds(poses, self.bounds, use_train_pose=True)
...
class DenseShinyDataset(ShinyDataset):
    ...
    def read_meta(self):
        ## Bounds
        with self.pmgr.open(os.path.join(self.root_dir, "bounds.npy"), "rb") as f:
            bounds = np.load(f)
        ...
        ## Poses
        with self.pmgr.open(os.path.join(self.root_dir, "poses.npy"), "rb") as f:
            poses = np.load(f)
