...
class FourierDataset(Dataset):
    ...
    def prepare_data(self):
        self.all_rays = self.all_rays.view(self.num_images, self.img_wh[1], self.img_wh[0], -1)
        self.all_rgb = self.all_rgb.view(self.num_images, self.img_wh[1], self.img_wh[0], -1)
        self.all_rgb_fft = fft_rgb(self.all_rgb)
        self.rgb_fft_mean = self.all_rgb_fft.mean(0)

    def shuffle(self):
        idx = list(np.random.choice(np.arange(0, self.num_images), size=self.num_images, replace=False))
...
class FourierLightfieldDataset(Dataset):
    ...
    def get_random_rays(self, ray_range):
        pos_rand = (torch.rand((2,)) * 2 - 1) * ray_range.pos

        return get_lightfield_rays(self.width, self.height, pos_rand[0], pos_rand[1], self.aspect)
