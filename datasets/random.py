...
class RandomRayDataset(Dataset):
    def __init__(self, cfg, train_dataset=None, **kwargs):
        super().__init__()
    ...
    def compute_stats(self):
        all_rays = self.all_rays.view(self.num_images, self.img_wh[1] * self.img_wh[0], -1)
    ...
    def get_random_rays(self, num_rays, ray_range):
        ...
        pos_rand = torch.randn((num_rays, ray_dim)) * self.pos_std[None] * ray_range.pos
        rays_o = self.pos_mean[None] + pos_rand

        dir_rand = torch.randn((num_rays, ray_dim)) * self.dir_std[None] * ray_range.dir
    ...
    def jitter_ray_origins(self, rays, jitter):
        ...
        pos_rand = (
            torch.randn((rays.shape[0], jitter.bundle_size, ray_dim), device=rays.device)
            * self.pos_std[None].type_as(rays)
            * jitter.pos
        )
    ...
    def jitter_ray_directions(self, rays, jitter):
        ...
        dir_rand = (
            torch.randn((rays.shape[0], jitter.bundle_size, ray_dim), device=rays.device)
            * self.dir_std[None].type_as(rays)
            * jitter.dir
        )
    ...
    def prepare_data(self, train_dataset):
        ...
        for i in range(train_dataset.num_images):
            ...
            cur_directions = get_ray_directions_from_pixels_K(
                cur_pixels, train_dataset.K, centered_pixels=train_dataset.centered_pixels
            )
            ...
            # Sample pixel colors
            cur_rgb = train_dataset.all_rgb.view(train_dataset.num_images, H, W, -1)[i].unsqueeze(0)
            cur_rgb = sample_images_at_xy(cur_rgb, cur_pixels, H, W)
            ...
            if self.use_ndc:
                self.all_ndc_rays.append(train_dataset.to_ndc(self.all_rays[-1]))
...
class RandomViewSubsetDataset(RandomRayDataset):
    def __init__(self, cfg, train_dataset=None, **kwargs):
        ...
        self.num_views = train_dataset.num_images if cfg.dataset.num_views == "all" else cfg.dataset.num_views
    ...
    def shuffle(self):
        ...
        self.current_rays = self.all_rays.view(self.num_images, self.img_wh[1] * self.img_wh[0], -1)[self.current_views]
        self.current_rgb = self.all_rgb.view(self.num_images, self.img_wh[1] * self.img_wh[0], -1)[self.current_views]
    ...
    def get_random_views(self, n_views):
        if self.num_views == self.num_images:
            ...
            return list(np.random.choice(np.arange(0, self.num_images), size=n_views, replace=False))

    def get_random_rays_convex_hull(self, num_rays, ray_range):
        rays = self.current_rays
        rays = rays[:, torch.randperm(rays.shape[1])]
        rays = rays[:, :num_rays]

        weights = torch.rand(num_rays, self.num_views).type_as(rays)
    ...
    def project_points(self, P, points):
        ...
        points = (P @ points.permute(0, 2, 1)).permute(0, 2, 1)
    ...
    def lookup_points(self, points):
        # Projection matrix
        poses_inv = torch.Tensor(self.current_poses_inv).type_as(points)[..., :3, :4]
        K = torch.Tensor(self.intrinsics).type_as(points).unsqueeze(0)
        P = K @ poses_inv
        ...
        # Valid mask
        valid_mask = (pixels[..., 0] > -1) & (pixels[..., 0] < 1) & (pixels[..., 1] > -1) & (pixels[..., 1] < 1)
        valid_mask = valid_mask.type_as(points).detach()[..., None]

        # Weights
        camera_centers = (
            torch.Tensor(self.current_poses).type_as(points)[..., None, :3, -1].repeat(1, points.shape[0], 1)
        )
        camera_dirs = torch.nn.functional.normalize(points.unsqueeze(0) - camera_centers, p=2.0, dim=-1)
        camera_rays = torch.cat([camera_centers, camera_dirs], dim=-1)

        # Lookup
        pixels = pixels.view(self.num_views, -1, 1, 2)
        rgb = self.current_rgb.permute(0, 2, 1).view(self.num_views, 3, self.img_wh[1], self.img_wh[0]).type_as(points)
        values = torch.nn.functional.grid_sample(rgb, pixels)
        values = values.permute(0, 2, 3, 1).reshape(self.num_views, -1, 3)
    ...
    def lookup_points_single(self, points, weights=None):
        ...
        # Weights
        camera_centers = (
            torch.Tensor(self.current_poses).type_as(points)[..., None, :3, -1].repeat(1, points.shape[1], 1)
        )
        camera_dirs = torch.nn.functional.normalize(points - camera_centers, p=2.0, dim=-1)
        camera_rays = torch.cat([camera_centers, camera_dirs], dim=-1)
...
class RandomRayLightfieldDataset(RandomRayDataset):
    ...
    def get_random_rays(self, num_rays, ray_range):
        st = (torch.rand((num_rays, 2)) * 2 - 1) * ray_range.pos

        s = st[..., 0] * self.st_scale
        t = st[..., 1] * self.st_scale

        uv = (torch.rand((num_rays, 2)) * 2 - 1) * ray_range.dir
        ...
        rays = torch.cat([rays[..., 0:3], torch.nn.functional.normalize(rays[..., 3:6], p=2.0, dim=-1)], -1)

        return rays

    def jitter_ray_directions(self, rays, jitter):
        dir_rand = torch.randn((rays.shape[0], jitter.bundle_size, 2), device=rays.device) * jitter.dir

        rays = rays.view(rays.shape[0], -1, rays.shape[-1])
        if rays.shape[1] == 1:
            rays = rays.repeat(1, jitter.bundle_size, 1)

        rays_d = torch.cat([rays[..., 3:5] + dir_rand.type_as(rays), rays[..., 5:]], -1)
    ...
    def jitter_ray_origins(self, rays, jitter):
        pos_rand = torch.randn((rays.shape[0], jitter.bundle_size, 2), device=rays.device) * jitter.pos * self.st_scale
