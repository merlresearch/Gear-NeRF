...
class MultipleNet(nn.Module):
    def __init__(self, in_channels, out_channels, cfg, **kwargs):
        ...
        if self.use_feature_net:
            ...
            self.feature_net = net_dict[net_cfg.type](
                self.out_feature_dim, self.out_channels, net_cfg, group=self.group
            )
    ...
    def forward(self, x, **render_kwargs):
        ...
        for idx, net in enumerate(self.nets):
            if self.cur_iter < self.wait_iters[idx] or self.cur_iter >= self.stop_iters[idx]:
                continue
