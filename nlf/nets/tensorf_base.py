...
class MLPRender_Fea(torch.nn.Module):
    def __init__(self, inChanel, viewpe=6, feape=6, featureC=128, out_dim=3):
        super().__init__()
        ...
        layer3 = torch.nn.Linear(featureC, out_dim)  # RGB + gear level
...
class TensorBase(torch.nn.Module):
    def __init__(self, in_channels, out_channels, cfg, **kwargs):
        ...
        self.nSamples = min(self.max_n_samples, cal_n_samples(self.gridSize, self.step_ratio))
        ...
        self.alphaMask_thres = cfg.alpha_mask_thre if "alpha_mask_thre" in cfg else 0.001
        self.rayMarch_weight_thres = cfg.rm_weight_mask_thre if "rm_weight_mask_thre" in cfg else 0.0001
    ...
    def init_render_func(self, shadingMode, pos_pe, view_pe, fea_pe, featureC, device):
        self.renderModule = MLPRender_Fea(self.app_dim, view_pe, fea_pe, featureC, 3).to(device)
        print("pos_pe", pos_pe, "view_pe", view_pe, "fea_pe", fea_pe)
        print(self.renderModule)
        self.renderModule_sam = MLPRender_Fea(self.app_dim, view_pe, fea_pe, featureC, 256).to(device)
        print("pos_pe", pos_pe, "view_pe", view_pe, "fea_pe", fea_pe)
        print(self.renderModule_sam)
    ...
    def update_stepSize(self, gridSize):
        ...
        self.nSamples = min(self.max_n_samples, int((self.aabbDiag / self.stepSize).item()) + 1)
    ...
    def getDenseAlpha(self, gridSize=None):
        ...
        for i in range(gridSize[0]):
            alpha[i] = self.compute_alpha(dense_xyz[i].view(-1, 3), 0.01).view((gridSize[1], gridSize[2]))
    ...
    def feature2density(self, density_features, **kwargs):
        if "weights" in kwargs:
            density_features = density_features * kwargs["weights"].view(*density_features.shape)
    ...
    def set_iter(self, iteration):
        ...
        if iteration in self.upsamp_list:
            ...
            self.nSamples = min(self.max_n_samples, cal_n_samples(reso_cur, self.step_ratio))
    ...
    def init_one_svd_density(self, n_component, gridSize, scale, device):
        ...
        for i in range(len(self.vecMode)):
            ...
            if self.fea2denseAct == "softplus":
                ...
                plane_coef.append(
                    torch.nn.Parameter(
                        scale * torch.rand((1, n_component[i], gridSize[mat_id_1], gridSize[mat_id_0])).clamp(1e-2, 1e8)
                    )
                )
                line_coef.append(
                    torch.nn.Parameter(scale * torch.rand((1, n_component[i], gridSize[vec_id], 1)).clamp(1e-2, 1e8))
                )
...
class TensorCP(TensorBase):
    ...
    def compute_densityfeature(self, xyz_sampled):
        ...
        line_coef_point = F.grid_sample(self.density_line[0], coordinate_line[[0]], align_corners=True).view(
    ...
    def compute_appfeature(self, xyz_sampled):
        ...
        line_coef_point = F.grid_sample(self.app_line[0], coordinate_line[[0]], align_corners=True).view(
            -1, *xyz_sampled.shape[:1]
        )
        line_coef_point = line_coef_point * F.grid_sample(
