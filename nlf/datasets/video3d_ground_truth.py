...
class Video3DTimeGroundTruthDataset(Base6DDataset):
    ...
    def read_meta(self):
        ...
        for i, pose_path in enumerate(self.pose_paths):
            ...
            if self.num_frames - 1 > 0:
                self.times.append((frame - self.start_frame) / (self.num_frames - 1))
    ...
    def format_batch(self, batch):
        ...
        batch["flow"] = batch["inputs"][..., self.all_coords.shape[-1] + 4 : self.all_coords.shape[-1] + 7]
    ...
    def load_geometry(self, idx, prefix="depth", mode="exr"):
        gt_path = os.path.join(self.root_dir, self.depth_paths[idx].replace("depth", prefix))
