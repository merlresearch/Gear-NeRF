...
class IntersectZPlane(Intersect):
    def __init__(self, z_channels, cfg, **kwargs):
        ...
        if self.use_disparity:
            self.samples = torch.linspace(
                1.0 / self.end, 1.0 / self.initial, z_channels // self.num_repeat, device="cuda"
            )

            self.samples = torch.flip(self.samples, [0])
        else:
            self.samples = torch.linspace(self.initial, self.end, z_channels // self.num_repeat, device="cuda")
    ...
    def intersect(self, rays, z_vals):
        ...
        dists = intersect_axis_plane(rays[..., None, :], z_vals, 2, exclude=False)
