...
class IntersectPlane(Intersect):
    def __init__(self, z_channels, cfg, **kwargs):
        ...
        if self.use_disparity:
            disparities = torch.linspace(1.0 / self.end, 1.0 / self.initial, z_channels, device="cuda")
            self.samples = torch.flip(disparities, [0])
        else:
            self.samples = torch.linspace(self.initial, self.end, z_channels, device="cuda")

        if z_channels > 1:
            self.z_scale = cfg.z_scale if "z_scale" in cfg else torch.abs(self.samples[1] - self.samples[0])
        else:
...
class IntersectEuclideanDistanceUnified(Intersect):
    def __init__(self, z_channels, cfg, **kwargs):
        ...
        self.samples = torch.linspace(self.initial, self.end, z_channels, device="cuda").view(-1, 1)
    ...
    def intersect(self, rays, distance):
        ...
        # Add distance offset
        distance = distance + (torch.sign(dot(rays_d, diff)) * torch.norm(diff, dim=-1)).unsqueeze(1)
    ...
    def __init__(self, z_channels, cfg, **kwargs):
        ...
        if self.use_dataset_bounds:
            self.initial = torch.tensor(
                cfg.initial if "initial" in cfg else kwargs["system"].dm.train_dataset.near * 1.5
            )
...
class IntersectCylinderNew(Intersect):
    def __init__(self, z_channels, cfg, **kwargs):
        ...
        if self.use_dataset_bounds:
            if cfg.outward_facing:
                ...
                self.initial = torch.tensor(
                    cfg.initial if "initial" in cfg else -kwargs["system"].dm.train_dataset.far * 1.5
                )
    ...
    def intersect(self, rays, z_vals):
        ...
        t = torch.where(torch.abs(radii) < min_radius + 4 * self.z_scale, raw_offsets + base_distance, t)
