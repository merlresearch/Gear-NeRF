...
class EPIVisualizer(BaseVisualizer):
    ...
    def validation(self, batch, batch_idx):
        ...
        if isinstance(dataset, LightfieldDataset) and dataset.keyframe_subsample == 1:
            all_rgb = dataset.all_rgb.view(dataset.num_rows, dataset.num_cols, dataset.img_wh[1], dataset.img_wh[0], 3)
        ...
        # Generate EPI rays
        rays = get_epi_rays(
            W, v, H, t, dataset.aspect, st_scale=self.st_scale, uv_scale=self.uv_scale, near=self.near, far=self.far
