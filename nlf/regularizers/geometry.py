...
from utils.ray_utils import dot, from_ndc, reflect
...
class GeometryRegularizer(BaseRegularizer):
    def __init__(self, system, cfg):
        ...
        # Contract function
        if "contract" in cfg:
            self.contract_fn = contract_dict[cfg.contract.type](cfg.contract, system=system)
        else:
    ...
    def _loss(self, batch, outputs, batch_idx):
        ...
        # Compute mask
        mask = (gt_depth != 0.0) & (pred_distance != 0.0)
    ...
    def _loss(self, batch, outputs, batch_idx):
        ...
        for idx, loss_weight in enumerate(self.weights):
            ...
            if self.teacher_fields[idx] == "render_normal":
                ...
                diff = 1.0 - dot(student_points, teacher_points)
                loss_match = (diff * cur_render_weights).sum((-2, -1)).mean()
                ...
                loss_penalty = (
                    (
                        torch.square(torch.maximum(dot_dirs_normal, torch.zeros_like(dot_dirs_normal)))
                        * cur_render_weights
                    )
                    .sum((-2, -1))
                    .mean()
...
class FlowRegularizer(BaseRegularizer):
    def __init__(self, system, cfg):
        ...
        # Contract function
        if "contract" in cfg:
            self.contract_fn = contract_dict[cfg.contract.type](cfg.contract)
        else:
            self.contract_fn = contract_dict["identity"]({})
    ...
    def _loss(self, batch, outputs, batch_idx):
        ...
        # Compute mask
        mask = (gt_flow != 0.0).any(dim=-1, keepdim=True) & (gt_depth != 0.0) & (pred_distance != 0.0)
    ...
    @property
    def render_kwargs(self):
        return {"fields": self.fields, "no_over_fields": self.fields, "no_flow_jitter": True}
