...
def sample_simplex(batch_size, n, device):
    ...
    samples = torch.cat([torch.zeros_like(samples[:, :1]), samples], dim=-1)
...
class RayDensityRegularizer(BaseRegularizer):
    def __init__(self, system, cfg):
        ...
        self.intersect_fn = intersect_dict[isect_cfg.type](self.z_channels, isect_cfg)
    ...
    def get_random_views(self, n_views, n_images):
        return list(np.random.choice(np.arange(0, n_images), size=n_views, replace=False))
    ...
    def _loss(self, train_batch, batch_results, batch_idx):
        ...
        with torch.no_grad():
            # Subset of cameras
            views_idx = torch.randint(
                0, self.dataset.num_images - 1, (self.batch_size * self.num_views_for_random,), device="cuda"
            )
            ...
            anchor_directions = get_ray_directions_from_pixels_K(pixels, self.K, centered_pixels=True).float()
            ...
            if (batch_idx % 3) == 1:
                ...
                anchor_positions = (anchor_positions - anchor_centroids) * self.extrapolate_scale + anchor_centroids
            ...
            if (batch_idx % 2) == 1:
                ...
                anchor_directions = torch.nn.functional.normalize(
                    (weights.unsqueeze(-1) * anchor_directions).sum(1), dim=-1
                )
            else:
            ...
            if self.use_jitter:
                anchor_positions = (
                    anchor_positions + torch.randn(anchor_positions.shape, device="cuda") * self.jitter_pos_std
                )
                anchor_directions = (
                    anchor_directions + torch.randn(anchor_directions.shape, device="cuda") * self.jitter_dir_std
                )
                anchor_directions = torch.nn.functional.normalize(anchor_directions, dim=-1)
            ...
            camera_dists = torch.linalg.norm(random_rays[..., None, :3] - centers, dim=-1)
            ...
            dirs = torch.nn.functional.normalize(camera_points, dim=-1)
        ...
        if (batch_idx % 3) == 0:
            ...
            weights = (
                ...
                * (
                    1
                    - torch.exp(
                        -torch.square(random_rays[..., :2]).mean(-1) + -torch.square(random_rays[..., 3:5]).mean(-1)
                    )
                )
                / N
            )
