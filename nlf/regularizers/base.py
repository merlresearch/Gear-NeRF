...
class BaseRegularizer(nn.Module):
    ...
    def _do_loss(self, name):
        return (
            self.cfg[name].wait_iters != "inf"
            and self.cur_iter >= self.cfg[name].wait_iters
            and self.cfg[name].weight > 0
        )

    def _loss_fn(self, name, *args):
        return self.loss_fns[name](*args) * self.cfg[name].weight
    ...
    def get_batch(self, train_batch, batch_idx, apply_ndc=False):
        ...
        use_inp = self.use_inp_freq == 0 or (
            (float(self.use_inp_freq) != float("inf")) and (batch_idx % self.use_inp_freq == 0)
        )
    ...
    def run_chunked(self, x):
        return render_chunked(x, self, {}, chunk=self.cfg.ray_chunk)
    ...
    def loss_weight(self):
        ...
        if isinstance(self.weight, float):
            ...
            weight_num_iters = (
                self.weight.num_epochs
                * len(system.trainer.datamodule.train_dataset)
                // system.trainer.datamodule.cur_batch_size
            )
