...
from nlf.activations import IdentityTanh, Power, Tanh, get_activation
...
class ZDepthContract(BaseContract):
    def __init__(self, cfg, **kwargs):
        ...
        if self.use_dataset_bounds:
            self.contract_end_radius = (
                cfg.contract_end_radius
                if "contract_end_radius" in cfg
                else kwargs["system"].dm.train_dataset.depth_range[1]
            )
        else:
...
class MIPNeRFContract(BaseContract):
    def __init__(self, cfg, **kwargs):
        ...
        if self.use_dataset_bounds:
            self.contract_start_radius = (
                cfg.contract_start_radius
                if "contract_start_radius" in cfg
                else max(kwargs["system"].dm.train_dataset.depth_range[0] * 1.5, 1.0)
            )
            self.contract_end_radius = (
                cfg.contract_end_radius
                if "contract_end_radius" in cfg
                else kwargs["system"].dm.train_dataset.depth_range[1] * 1.5
            )
        else:
        ...
        self.contract_start_distance = (
            cfg.contract_start_distance if "contract_start_distance" in cfg else self.contract_start_radius
        )
        self.contract_end_distance = (
            cfg.contract_end_distance if "contract_end_distance" in cfg else self.contract_end_radius
        )

        if "distance_activation" in cfg:
            self.distance_activation = get_activation(cfg.distance_activation)
        else:
    ...
    def inverse_contract_distance(self, distance):
        ...
        return (
            torch.where(torch.abs(distance) < 1, distance, torch.sign(distance) * (1.0 / inverse_distance))
            * self.contract_start_distance
        )
    ...
    def contract_points(self, points):
        ...
        return torch.where(distance < 1, points, (points / distance) * (2.0 - t))
    ...
    def __init__(self, cfg, **kwargs):
        ...
        if self.use_dataset_bounds:
            self.contract_start_radius = (
                cfg.contract_start_radius
                if "contract_start_radius" in cfg
                else max(kwargs["system"].dm.train_dataset.depth_range[0] * 1.75, 1.0)
            )
            self.contract_end_radius = (
                cfg.contract_end_radius
                if "contract_end_radius" in cfg
