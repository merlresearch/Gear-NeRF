...
from utils.intersect_utils import intersect_axis_plane, intersect_cylinder, intersect_sphere
...
class TwoPlaneParam(nn.Module):
    ...
    def forward(self, rays):
        ...
        t1 = intersect_axis_plane(rays, self.near, 2)

        t2 = intersect_axis_plane(rays, self.far, 2)
...
def multi_plane_param(cfg, **kwargs):
    ...
    depth_samples = torch.linspace(initial_z, end_z, z_channels, device="cuda") * voxel_size

    def param(rays):
        t = intersect_axis_plane(rays[..., None, :], depth_samples[None], 2, exclude=False)
...
def two_plane_matrix(cfg, **kwargs):
    ...
    def param(rays):
        ...
        # Intersect
        isect_pts_1, _ = intersect_axis_plane(rays, near_z, 2, exclude=False)

        isect_pts_2, _ = intersect_axis_plane(rays, far_z, 2, exclude=False)
...
def two_plane_pos(cfg, **kwargs):
    ...
    def pos(rays):
        ...
        isect_pts, _ = intersect_axis_plane(rays, near, 2, exclude=False)
    ...
    def forward(self, rays):
        ...
        return torch.cat(
            [
                ...
                self.contract_fn.contract_points(
                    param_rays[..., self.contract_start_channel : self.contract_end_channel]
                ),
...
def spherical_param(cfg, **kwargs):
    def param(rays):
        isect_pts = intersect_sphere(rays, cfg.radius) / cfg.radius
...
def two_cylinder_param(cfg, **kwargs):
    ...
    def param(rays):
        isect_pts_1, _ = intersect_cylinder(rays, origin[None], near, sort=False)
        isect_pts_2, _ = intersect_cylinder(rays, origin[None], far, sort=False)
...
time_param_dict = {"identity": identity}
