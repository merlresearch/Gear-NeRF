...
def rgb_ssim(img0, img1, max_val, filter_size=11, filter_sigma=1.5, k1=0.01, k2=0.03, return_map=False):
    ...
    filt_fn = lambda z: np.stack(
        [convolve2d(convolve2d(z[..., i], filt[:, None]), filt[None, :]) for i in range(z.shape[-1])], -1
    )
    mu0 = filt_fn(img0)
    mu1 = filt_fn(img1)
    mu00 = mu0 * mu0
...
def RGBtFourierRender(xyz_sampled, viewdirs, features, kwargs):
    ...
    freqs = torch.linspace(0, frames_per_keyframe - 1, frames_per_keyframe, device=time_offset.device)[None]
    basis = torch.cat(
        [
            t,
