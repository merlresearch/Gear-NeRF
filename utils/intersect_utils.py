...
def intersect_sphere(rays, origin, radius, continuous=False):
    ...
    t1 = torch.where(disc <= 0, torch.zeros_like(t1), t1)
    t2 = torch.where(disc <= 0, torch.zeros_like(t2), t2)

    t = torch.where((t2 < 0) | (radius < 0), t1, t2)
...
def intersect_cylinder(rays, origin, radius, continuous=False):
    ...
    t = torch.where((t2 < 0) | (radius < 0), t1, t2)  # TODO: Maybe change
...
def intersect_axis_plane(
    ...
    rays_d = torch.where(torch.abs(rays_d) < 1e-5, torch.ones_like(rays_d) * 1e12, rays_d)
...
def intersect_max_axis_plane(
    ...
    t = (val - rays_o) / rays_d
    t = torch.where(t < 1e-8, torch.zeros_like(t), t)
...
def intersect_plane(
    ...
    d_dot_n = torch.where(torch.abs(d_dot_n) < 1e-5, torch.ones_like(d_dot_n) * 1e12, d_dot_n)
